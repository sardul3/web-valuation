...
def edit_evaluation_student(request,evaluation_id):
    ...
    print(evaluation_found.evaluated_by)
    print(Evaluator.objects.filter(name=evaluation_found.evaluated_by))
    email_eval = Evaluator.objects.filter(email=evaluation_found.evaluated_by)[0]
    mystudent=0
    for stu in custom_students.objects.all():
...
def generate_cycle_report(request, cycle_id):
    ...
    for outcome in outcomes:
        ...
        for measure in measures:
            evaluated_student_count = custom_students.objects.filter(measure=measure, grade__isnull = False, current=True).count()
            number_of_pass_cases = custom_students.objects.filter(measure=measure,grade__gte = measure.cutoff_score, current=True).count()
            data.update({outcome.id: [evaluated_student_count, number_of_pass_cases,  outcome.title, me.extend([measure.measureTitle,  measure.statusPercent, measure.status])]})
    print(data)
    context ={'outcomes':outcomes, 'measures':measures,  'data':data, 'cycle_id': cycle_id,
    'count':measures.count()+1}
